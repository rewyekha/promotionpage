// projectMaintenanceRoutes.js - Backend routes for Project Maintenance functionality (with mock data for testing)
...
const mockModules = [
    { ModuleNo: 1, Description: "General Ledger" },
    { ModuleNo: 2, Description: "Accounts Payable" },
    { ModuleNo: 3, Description: "Accounts Receivable" }
];

const mockProjectTypes = [
    { ProjectTypeID: "EXE", Description: "Executable" },
    { ProjectTypeID: "DLL", Description: "Dynamic Link Library" },
    { ProjectTypeID: "RPT", Description: "Report" }
];

const mockDevelopers = [
    { DeveloperID: "admin", DeveloperName: "Administrator", BuildRole: "Administrator" },
    { DeveloperID: "dev1", DeveloperName: "John Developer", BuildRole: "Developer" },
    { DeveloperID: "pm1", DeveloperName: "Jane Manager", BuildRole: "Project Manager" }
];

const mockSSLocations = [
    { SSLocation: "Main" },
    { SSLocation: "Branch" },
    { SSLocation: "Development" }
];

const mockProjects = [
    {
        ID: 1,
        ProjectFileName: "TestProject1.exe",
        Description: "Sample Test Project 1",
        DeveloperName: "John Developer",
        ProjectTypeDescription: "Executable",
        ModuleNo: 1,
        ProjectTypeID: "EXE",
        DeveloperID: "dev1",
    ...
    {
        ID: 2,
        ProjectFileName: "TestProject2.dll",
        Description: "Sample Test Project 2",
        DeveloperName: "Jane Manager",
        ProjectTypeDescription: "Dynamic Link Library",
        ModuleNo: 2,
        ProjectTypeID: "DLL",
        DeveloperID: "pm1",
        ProjectManagerID: "pm1"
    ...
    try {
        ...
        try {
            ...
            if (moduleNo && moduleNo !== '0') {
                filteredProjects = filteredProjects.filter(p => p.ModuleNo == moduleNo);
            }
            
            if (projectType && projectType !== 'All') {
                filteredProjects = filteredProjects.filter(p => p.ProjectTypeID === projectType);
            }
            
            if (projectMask) {
                const mask = projectMask.endsWith('%') ? projectMask.slice(0, -1) : projectMask;
                filteredProjects = filteredProjects.filter(p => 
                    p.ProjectFileName.toLowerCase().includes(mask.toLowerCase())
                );
            }
            
            if (descriptionMask) {
                const mask = descriptionMask.endsWith('%') ? descriptionMask.slice(0, -1) : descriptionMask;
                filteredProjects = filteredProjects.filter(p => 
                    p.Description.toLowerCase().includes(mask.toLowerCase())
                );
            }
        ...
        if (projectMask) {
            // Add % to end if not present (matching ASP logic)
...
router.post('/project', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.log('Database not available, simulating project creation');
            const newId = Math.max(...mockProjects.map(p => p.ID)) + 1;
            return res.json({ id: newId, message: 'Project created successfully (mock)' });
        }
...
router.put('/project/:id', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.log('Database not available, simulating project update');
            return res.json({ message: 'Project updated successfully (mock)' });
        }
...
router.delete('/project/:id', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.log('Database not available, simulating project deletion');
            return res.json({ message: 'Project deleted successfully (mock)' });
        }
