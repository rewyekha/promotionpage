...
router.get("/RoleAssignment", async (req, res) => {
  try {
    ...
    console.log("LOGON_USER_RoleAssignment:", logonUser);
    console.log("Extracted devId_RoleAssignment:", devId);
    ...
    // === CASE 2: No Rights (role not Administrator) ===
    if (buildRole !== "Administrator") {
      const filePath = path.join(__dirname, "../public/ErrorPages/NoRights.html");
      return fs.readFile(filePath, "utf8", (err, data) => {
        if (err) return res.status(500).send("Error loading NoRights page");
    ...
    res.sendFile(path.join(__dirname, "../public/RoleAssignment.html"));
  } catch (error) {
    console.error("Error in /RoleAssignment route:", error);
