...
router.get("/DeveloperAssignment", async (req, res) => {
  try {
    ...
    console.log("LOGON_USER_DeveloperAssignment:", logonUser);
    console.log("Extracted devId_DeveloperAssignment:", devId);
    ...
    // === CASE 2: No Rights (role not Administrator or Project Manager) ===
    if (buildRole !== "Administrator" && buildRole !== "Project Manager" && buildRole !== "Developer") {
      const filePath = path.join(__dirname, "../public/ErrorPages/NoRights.html");
      return fs.readFile(filePath, "utf8", (err, data) => {
        if (err) return res.status(500).send("Error loading NoRights page");
    ...
    res.sendFile(path.join(__dirname, "../public/DeveloperAssignment.html"));
  } catch (error) {
    console.error("Error in /DeveloperAssignment route:", error);
